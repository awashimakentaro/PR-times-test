...
## なぜ効くか
- users(account_name, del_flg): アカウント名＋削除フラグでピンポイント検索でき、名簿を最初から全部めくらなくてよくなる。
- comments(post_id, created_at): ある投稿のコメントが日時順に並んだ「目次」ができ、新しい3件は先頭から3つ取るだけ（並べ替え不要、無駄読み減）。
